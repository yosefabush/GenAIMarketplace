...
:: Ralph Production Server Launcher
:: Starts backend (port 8000) and frontend (port 5173) in production mode
:: Restarts if already running, logs all activity

set SCRIPT_DIR=%~dp0
set LOG_FILE=%SCRIPT_DIR%prod-server.log
set BACKEND_PORT=8000
set FRONTEND_PORT=5173

:: Initialize log file
echo ============================================== >> "%LOG_FILE%"
echo [%date% %time%] Starting production servers >> "%LOG_FILE%"
echo ============================================== >> "%LOG_FILE%"

echo.
echo =============================================
echo   Ralph Production Server Launcher
echo =============================================
echo.
...
echo.
echo [%time%] Starting backend server (docker compose production)...
echo [%date% %time%] Starting backend server via docker-compose.prod.yml >> "%LOG_FILE%"

:: Start backend with docker compose production in new window
cd /d "%SCRIPT_DIR%backend"
start "Ralph Backend (Production)" cmd /k "echo Starting FastAPI backend on port %BACKEND_PORT% via Docker production compose... && docker-compose -f docker-compose.prod.yml up --build"

:: Wait for backend container to initialize
timeout /t 5 /nobreak >nul

echo [%time%] Starting frontend server (production build)...
echo [%date% %time%] Starting frontend server in production mode >> "%LOG_FILE%"

:: Start frontend in new window with production build
cd /d "%SCRIPT_DIR%frontend"
start "Ralph Frontend (Production)" cmd /k "echo Building and starting Vite frontend in production mode on port %FRONTEND_PORT%... && npm run build && npm run preview -- --port %FRONTEND_PORT%"
...
echo.
echo [%date% %time%] Production servers started successfully >> "%LOG_FILE%"
